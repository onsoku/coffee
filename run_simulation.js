...
function printReport(result, title) {
  ...
  output += '--------------------------------------------------\n';
  output += '■ 焙煎イベント :\n';
  output += ` - 水分抜け(150C) : ${result.events.dryEnd ? Math.floor(result.events.dryEnd / 60) + 'm ' + (result.events.dryEnd % 60) + 's' : '到達せず'}\n`;
  output += ` - １ハゼ  (200C) : ${result.events.firstCrack ? Math.floor(result.events.firstCrack / 60) + 'm ' + (result.events.firstCrack % 60) + 's' : '到達せず'}\n`;
  output += ` - ２ハゼ  (225C) : ${result.events.secondCrack ? Math.floor(result.events.secondCrack / 60) + 'm ' + (result.events.secondCrack % 60) + 's' : '到達せず'}\n`;
  output += '--------------------------------------------------\n';
  
  // 欠点の警告
  let defectMessages = [];
  if (result.defects.underdeveloped) defectMessages.push('⚠️ 未発達 (Underdeveloped): 焙煎が進んでいません！');
  if (result.defects.scorched)       defectMessages.push('🔥 スコーチ (Scorched): 火力が強すぎて表面が焦げています！');
  if (result.defects.baked)          defectMessages.push('🥱 ベイクド (Baked): 焙煎に時間がかかりすぎ、風味が飛んでいます！');
  ...
  logScore('香り (Aroma)', result.scores.aroma);
  logScore('風味 (Flavor)', result.scores.flavor);
  logScore('後味 (Aftertaste)', result.scores.aftertaste);
  logScore('酸味 (Acidity)', result.scores.acidity);
  logScore('ボディ (Body)', result.scores.body);
  logScore('甘さ (Sweetness)', result.scores.sweetness);
  logScore('クリーンカップ', result.scores.cleanCup);
  output += `  ★ 総合評価      : ${result.scores.overall} 点\n`;
  output += '==================================================\n\n';
 ...
 */
process.on('exit', () => {
  console.log(`✅ シミュレーション完了。結果は ${REPORT_FILE} に出力されました。`);
});
